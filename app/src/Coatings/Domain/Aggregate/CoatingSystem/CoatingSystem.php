...
class CoatingSystem extends Aggregate
{
    ...
    private function assertLayersAreChainable(): void
    {
        ...
        $n = count($layers);
        for ($i = 0; $i < $n - 1; ++$i) {
            $lower = $layers[$i]->getCoating();
            $upper = $layers[$i + 1]->getCoating();
            if (!$lower->canBecoveredBy($upper)) {
                $lowerBase = $lower->getBase();
                $upperBase = $upper->getBase();
                throw new AppException(sprintf('Слой %d (%s) несовместим со слоем %d (%s): поверх %s нельзя наносить %s.', $i + 1, $lowerBase->title(), $i + 2, $upperBase->title(), $lowerBase->title(), $upperBase->title()));
            }
        }
    }
