    ...
    public function getValidationCollectionGeneralProposalInfo(): Assert\Collection
    {
        return new Assert\Collection(fields: [
            'number' => [
                ...
                new Assert\Length(min: 1, max: 100, maxMessage: 'Номер не должен быть длиннее {{ limit }}.', minMessage: 'Номер не должен быть короче {{ limit }}.'),
            ],
            'description' => new Assert\Optional([
                new Assert\NotBlank(),
                new Assert\Type('string'),
                new Assert\Length(min: 3, max: 750, maxMessage: 'Описание не должно быть длиннее {{ limit }}.', minMessage: 'Описание не должно быть короче {{ limit }}.'),
            ]),
            'basis' => new Assert\Optional([
                new Assert\NotBlank(),
                new Assert\Type('string'),
                new Assert\Length(min: 3, max: 750, maxMessage: 'Основание не должно быть длиннее {{ limit }}.', minMessage: 'Основание не должно быть короче {{ limit }}.'),
            ]),
            'projectArea' => [
                new Assert\NotBlank(),
                new Assert\Type('numeric'),
                new Assert\Range(min: 1, max: 1000000, notInRangeMessage: 'Площадь должна быть в переделах от {{ min }} до {{ max }} м кв.'),
            ],
            'loss' => [
                new Assert\NotBlank(),
                new Assert\Type('numeric'),
                new Assert\Range(min: 0, max: 99, notInRangeMessage: 'Потери должны быть в переделах от {{ min }} до {{ max }} %.'),
            ],
            'projectTitle' => [
                new Assert\NotBlank(),
                new Assert\Type('string'),
                new Assert\Length(min: 3, max: 750, maxMessage: 'Название проекта не должно быть длиннее {{ limit }}.', minMessage: 'Название проекта не должно быть короче {{ limit }}.'),
            ],
            'projectStructureDescription' => new Assert\Optional([
                new Assert\NotBlank(),
                new Assert\Type('string'),
                new Assert\Length(min: 3, max: 750, maxMessage: 'Описание конструкций проекта не должно быть длиннее {{ limit }}.', minMessage: 'Описание конструкций проекта не должно быть короче {{ limit }}.'),
            ]),
            'durability' => [
                new Assert\NotBlank(),
                new Assert\Type('string'),
                new Assert\Choice(choices: CoatingSystemDurability::values(),
            ...
            'category' => [
                new Assert\NotBlank(),
                new Assert\Type('string'),
                new Assert\Choice(choices: CoatingSystemCorrosiveCategory::values(),
            ...
            'treatment' => [
                new Assert\NotBlank(),
                new Assert\Type('string'),
                new Assert\Choice(choices: CoatingSystemSurfaceTreatment::values(),
            ...
            'method' => [
                new Assert\NotBlank(),
                new Assert\Type('string'),
                new Assert\Choice(choices: CoatingSystemApplicationMethod::values(),
            ...
            'coats' => new Assert\All([
                new Assert\Collection(fields: [
                    ...
                    'coatDft' => [
                        new Assert\NotBlank(),
                        new Assert\Type('numeric'),
                        new Assert\Range(min: 10, max: 9999, notInRangeMessage: 'ТСП должна быть в переделах от {{ min }} до {{ max }}.'),
                    ],
                    'coatColor' => [
                        new Assert\NotBlank(),
                        new Assert\Type('string'),
                        new Assert\Length(min: 3, max: 100, maxMessage: 'Название цвета покрытия не должно быть длиннее {{ limit }}.', minMessage: 'Название цвета покрытия не должно быть короче {{ limit }}.'),
                    ],
                    'thinnerPrice' => [
                        new Assert\NotBlank(),
                        new Assert\Type('numeric'),
                    ...
                    'thinnerConsumption' => [
                        new Assert\NotBlank(),
                        new Assert\Type('numeric'),
                        new Assert\Range(min: 0, max: 99, notInRangeMessage: 'Процент разбавления должен быть в переделах от {{ min }} до {{ max }} %.'),
                    ],
                ]),
            ]),
        ],
