...
final readonly class CasNumber implements \Stringable
{
    ...
    public static function fromString(string $raw): self
    {
        if (!preg_match('/^(\d{2,7})-(\d{2})-(\d)$/', $raw, $m)) {
            throw new AppException(sprintf('Неверный формат CAS-номера «%s». Ожидается NNNNNNN-NN-N.', $raw));
        }
        ...
        if ($sum % 10 !== (int) $checkDigit) {
            throw new AppException(sprintf('Неверная контрольная цифра CAS-номера «%s».', $raw));
        }
