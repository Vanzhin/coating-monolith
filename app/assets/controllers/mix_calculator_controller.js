...
export default class extends Controller {
    ...
    /**
     * Пропускает в поле только цифры и один разделитель. Мобильная клавиатура по локали шлёт
     * запятую — нормализуем её в точку до фильтрации, иначе дробь не набрать. Для части
     * ограничивает число знаков после запятой ($maxDecimals), для количества — без ограничения
     * (null). Переприсваиваем значение только когда реально что-то вырезали, чтобы не дёргать
     * каретку при обычном вводе.
     */
