# [УСТАРЕЛ] План 1: наполнение read-модели

**Этот план заменён.** Актуальная версия — `plan-1-domain-and-cache-redesign.md`.
Причина переделки — правило проекта «домен vs инфраструктура»: производные величины считаются
runtime-методами агрегата, кэш живёт в отдельных таблицах и обновляется event-driven через
subscriber-ы (не через Doctrine listener и не через поля агрегата).
