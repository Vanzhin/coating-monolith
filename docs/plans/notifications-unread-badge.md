...
- **Рассылка — не в хендлере, а через доменное событие (async).** `Notification` в конструкторе поднимает `NotificationCreatedEvent(notificationId)` (только id — как `ChannelVerifiedEvent`); на сохранении `PublishDomainEventsOnFlushListener` публикует его, событие роутится на `async` (messenger.yaml) → `NotificationCreatedEventHandler` в воркере **читает уведомление из БД по id** и фан-аутит: `channelRepository->findByOwnerAndType($notification->getOwnerUlid(), WEB_PUSH)` → на каждый `channelNotifierService->notify($channel, $notification->getMessage())`. I/O-доставка уходит из запроса.
- **Publish-before-commit решён re-fetch'ем.** Событие публикуется на `postFlush` до COMMIT транзакции команды. Обработчик читает строку по id: если не видно (не закоммичена/откатилась) — бросает, и Messenger повторяет (дефолт: 3 ретрая, backoff 1с/2с/4с). Значит доставка и `countUnread` идут по видимой строке (бейдж без недосчёта), а откат не даёт «призрачный» пуш. Остаточное: сам XADD в Redis происходит в `postFlush` (связка запись↔Redis) — отдельная кросс-модульная тема (dispatch-after-commit / outbox), не в этой фиче.
