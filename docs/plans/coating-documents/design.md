...
- **Гибкий поиск** документов по `title`/`subject`/`description` — через Postgres FTS (tsvector),
  не LIKE.
- **Заморозка сертифицированной системы (целиком)**: если к системе привязан хотя бы один документ
  (`Reference` типа `CoatingSystem`), её нельзя менять ВООБЩЕ — ни слои (состав/толщина/цвет), ни
  подложку/подготовку/среду/название/теги. Удаление такой системы тоже запрещено, пока документы не
  отвязаны/удалены. Без слепков/диффа — жёсткий запрет. Сообщение направляет пользователя: «Создайте
  новую систему (при необходимости дублированием) и правьте её сколько угодно.» Механизм — write-side
  guard во ВСЕХ командных хендлерах мутаций системы Coatings через кросс-контекстный порт
  Coatings←Certificates (`hasCertificates(systemId)`), без денормализованного флага. Это дополнение
  к Плану 3 (тот же порт, что и read-модель). Агрегат `CoatingSystem` правило сам не проверяет —
  оно про внешний факт (документ в другом контексте).
