...
**Критерий «домен vs инфраструктура»**: спроси «это часть того, чем сущность является в реальном мире, или это техника/удобство для её хранения/поиска?»

- Часть сущности → **домен**. Пример: у системы покрытий «время сборки при 20 °C» и «список удовлетворяемых стандартов» — это её свойства как объекта, они меняются с изменением слоёв, влияют на решения бизнеса. Живут как поля агрегата, пересчитываются автоматически при мутации (в `postMutate`/сеттерах).
- Техника хранения/поиска/сериализации → **инфраструктура**. Пример: PostgreSQL-tsvector для полнотекстового поиска, специфические денормализованные таблицы для быстрого фильтра, форматы сериализации, кэши. Живёт в `{Context}/Infrastructure/` (проекторы, репозитории, DBAL-типы), обновляется явно после сохранения агрегата.

Правило проверки: если удалить эту технологию (перевести проект на другую БД, поменять индекс) — исчезнет ли смысл сущности? Если нет — это инфраструктура, ей не место в домене.
...
**3. Человекочитаемый код, особенно в доменах.** Код читают чаще, чем пишут. В доменных объектах и там, где встречается предметная логика, читатель должен понимать смысл без разбора внутренностей. Признаки нечитаемого:

- Гроздь условий через `||`/`&&` без имени: `if (null === $point || $point->isCalculated || null === $point->timeInMinutes || $point->timeInMinutes <= 0)`. Что это значит бизнесово — приходится расшифровывать по частям. Вынести в говорящий метод-предикат (`hasExplicitPositivePointAt(20)`) или в промежуточную переменную с именем факта.
- Обращение к чужим полям через цепочку (`$layer->getCoating()->getMinRecoatingInterval()->default->getPoint(20)`) — Application/Infrastructure лезет в потроха домена. Вынести операцию на владельца данных: метод на агрегате/VO, который сам применяет правило и возвращает готовый результат.
- Магические числа/строки в бизнес-выражениях — заменить именованной константой или enum-значением.

Правило: если фрагмент нельзя прочитать вслух и понять, что он делает по смыслу — переписать. Лишние 5-10 строк на говорящий метод почти всегда окупаются.


## Главное правило
**Автономный режим.** Существенную задачу вести самостоятельно через многоагентную оркестрацию, а не пошаговым согласованием. Рой прорабатывает план → реализует → ревьюит сам; пользователю — «готово» и только реальные развилки. Пошаговое «обсуди и получи подтверждение перед каждым шагом» — не применяем.
...
Обязательная последовательность на каждую задачу. Не опционально и не «если вспомнил».

1. **Создаём ветку по номеру задачи**.
    Не работаем в `main` напрямую.
2. **Делаем.** Коммиты — точечно, партиями по смыслу: на большую фичу нормально 3–4 коммита,
   не сваливать всё в один. Формат сообщения — раздел «Коммиты» ниже.
   **Коммитить только по явному апруву разработчика.** Произвольных коммитов «по ходу» не делаем:
   правки живут в рабочем дереве, пока не сказано «коммить». То же про пуш.
3. **Пушим ветку** — тоже по апруву.

Задача в трекере обязана отражать реальность на каждом шаге.

## Коммиты

Релиз-ноты собирает отдельный контейнер, он берёт сообщения коммитов как есть и **влезает
примерно в 3000 символов на весь релиз**. Поэтому многострочные простыни в коммитах запрещены —
они съедают лимит и релиз обрезается.

Формат — **одна строка**, без тела:

- `ID задачи` + пробел + текст. Без двоеточия после ID.
- **Максимум 150 символов**, целимся в 100.
- Заголовок в духе жёлтой прессы: кричащий, конкретный, цепляющий — но по делу и понятный
  тому, кто в задаче не сидел. Не «поправил логику», а что именно поменялось для системы.
- Тело коммита — только если без него совсем никак (миграция данных, разовая инструкция).
  Обоснования, разборы и «почему так» — в план `docs/plans/`, не в коммит.
- Нет ID задачи (инфраструктура, автоген) — начинаем сразу с текста.

Хорошо:
```
Один API-ключ превратился в список токенов со сроком жизни и отзывом
Забаненный юзер больше не ходит по API с валидным JWT — дыру закрыл единый guard
```

## Куда писать правила

Корневой CLAUDE.md — только кросс-модульное: процесс работы, роли, команды `./run`. Правила конкретного модуля — в его CLAUDE.md: `modules/backend/CLAUDE.md`, `modules/frontend/CLAUDE.md`, `modules/landing-v2/CLAUDE.md`. Перед записью нового правила определи модуль и пиши туда.
...
- **Не изобретай стили — копируй из ближайшего аналога.** UI единообразный: любую новую страницу/карточку/модалку начинай с поиска существующего похожего шаблона (для admin — `Templates/admin/coating/coating/`, для cabinet — соответствующий `Templates/cabinet/coating/`), копируй разметку 1-в-1, меняешь только данные и тексты. Никаких новых CSS-классов, никаких новых цветов/spacing/иконок кроме уже используемых `bi-*`. Если аналога нет — спроси, чем руководствоваться.
...
### `./run` — хаб команд проекта

Единая точка запуска для всего, что гоняем руками. **Все полезные команды складываем сюда**,
а не плодим отдельные скрипты в корне. `./run help` показывает список (описания берутся из
`#:`-комментариев в самом скрипте). Добавить команду — одна ветка в `case` + строка `#:`.

```bash
./run help              # список команд
./run db-pull-prod      # снять полный дамп прод-БД и раскатать локально (флаги: -h)
./run console <cmd>     # bin/console в контейнере php-fpm
```
