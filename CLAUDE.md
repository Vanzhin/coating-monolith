...
- **Не делай `git commit`/`git add`.** Пользователь сам управляет коммитами. **Исключение:** режим subagent-driven-development (SDD) — implementer каждой SDD-задачи коммитит после успешного зелёного теста, потому что review между задачами работает по diff-ам конкретных коммитов. Признак SDD-режима — dispatch prompt явно содержит фразу «SDD-режим» и/или ссылку на `.superpowers/sdd/*/task-N-brief.md`; тогда `git add`/`git commit` разрешены (message: `feat(...)`/`fix(...)`/`refactor(...)`, английский). Вне SDD правило соблюдать буквально.
